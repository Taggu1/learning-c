...
void init_database() {
    parts = malloc((sizeof(Part) * current_max));
}

void make_database_bigger() {
    current_max *= 2;
    parts = realloc(parts, current_max);
}

void insert_part() {
    // Check if database is full

    if (parts == NULL) init_database();

    if (parts_count == current_max) make_database_bigger();
...
void print_parts() {
    qsort(parts, parts_count, sizeof(parts[0]), compare_parts);
